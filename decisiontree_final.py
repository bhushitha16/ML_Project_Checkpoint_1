...
# Optional: check/remove duplicates
...
logging.info(f"Best Decision Tree parameters: {search.best_params_}")

# ------------------------
# Validation performance
# ------------------------
y_pred_val_log = best_dt.predict(X_val)
y_pred_val = np.expm1(y_pred_val_log)  # inverse of log1p

rmse_val = np.sqrt(mean_squared_error(y_val, y_pred_val))
r2_val = r2_score(y_val, y_pred_val)
logging.info(f"Validation RMSE (original scale): {rmse_val:.2f}")
logging.info(f"Validation R2 Score (original scale): {r2_val:.3f}")
...
logging.info("Predicting with Decision Tree...")
y_test_pred_log = best_dt.predict(X_test_features)
...
output_df.to_csv('test_transport_predictions_dt_log.csv', index=False)
logging.info("Saved predictions to 'test_transport_predictions_dt_log.csv'")
