...
for name, model in models.items():
    ...
    search = RandomizedSearchCV(pipeline, param_distributions=param_grids[name], n_iter=9, cv=3,
...
logging.info(f"RMSE: {best_score:.2f}")
...
logging.info(f"Validation R2 Score: {r2:.3f}")
...
output_df.to_csv('test_transport_predictions.csv', index=False)
logging.info("Saved predictions to 'test_transport_predictions.csv'")